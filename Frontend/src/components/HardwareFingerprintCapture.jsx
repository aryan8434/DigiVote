  ...
  if (!supported) {
    return (
      <div className="p-4 rounded-2xl bg-rose-500/10 border border-rose-500/20 text-sm text-rose-400 backdrop-blur-md flex items-start gap-3">
        ...
        <p>
          Browser version does not support Web Serial API. Please use Chrome or
          Edge for hardware integration.
        </p>
      </div>
    );
  }
      ...
      <div className={doneClasses}>
        ...
        <div>
          ...
          <p className="text-sm text-emerald-400/80 font-medium">
            Verified via external hardware sensor
          </p>
        </div>
      </div>
    );
    ...
    <div className="space-y-6">
      <div className="flex items-start gap-4 p-5 rounded-3xl bg-slate-950/40 border border-slate-800/60 backdrop-blur-md relative overflow-hidden group">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-slate-400 leading-relaxed max-w-md">
            Compatible with DigiVote external biometric modules. Ensure your
            USB-Serial device is recognized before connecting.
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-4 items-center">
        ...
        <div className="flex items-center gap-2 ml-auto">
          <div
            className={`h-2 w-2 rounded-full ${isNative ? (done ? "bg-emerald-500" : "bg-slate-700") : port ? "bg-emerald-500 animate-pulse" : "bg-slate-700"}`}
          />
